...
def candidatos(request):
    dic = {'candidatos': get_candidatos(request), 'zonas': request.session['configuracao'].keys()}
    return render(request, 'candidatos.html', dic)


def get_sessoes(request):
    try:
        return JsonResponse(dados_sessao(request))
...
def dados_sessao(request):
    ...
    return {"sessoes": sessoes, 'qtdVotosApurados': qtdZonas, 'qtdAbstencaoApurados': qtd_abstencao,
            'status': 200,
...
def get_candidattos_filtro(request):
    retorno = {'status': 200,
               'mensagem': ''}

    try:
        if (not request.GET.get('zona') and not request.GET.get('sessao')) or not request.GET.get('zona'):
            candidatos = get_candidatos(request)
        elif request.GET.get('zona') and request.GET.get('sessao'):
            candidatos = get_candidatos(request, f'WHERE zona = "{request.GET.get('zona')}" and sessao = "{request.GET.get('sessao')}"')
        else:
            candidatos = get_candidatos(request, f'WHERE zona = "{request.GET.get('zona')}"')
        contexto = {
            'candidatos': candidatos
        }
        html = render_to_string('template_candidato.html', contexto)
        retorno = dados_sessao(request)
        retorno['html'] = html
        return JsonResponse(retorno)
    except KeyError as e:
        erro = f"A chave '{e.args[0]}' não foi encontrada na apuração dos votos."
